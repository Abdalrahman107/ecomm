...
const ForgetPassword = () => {
    ...
    async function verifyPassword(values){
        try{
           const { data } = await axios.post('https://ecommerce.routemisr.com/api/v1/auth/forgotPasswords',values)
            navigate('/verificationresetcode')
    ...
    const formik = useFormik({
        ...
        onSubmit:(values)=>verifyPassword(values),
  ...
  return (
    <div className='container mt-20'>
        ...
        <form onSubmit={formik.handleSubmit} className="mt-4">
            ...
            <button type="submit" className="text-white bg-green-500 hover:bg-green-400 cursor-pointer font-medium rounded-lg text-sm w-full sm:w-auto block ml-auto px-8 py-2.5 text-center">Get Verification Code</button>
        </form>
